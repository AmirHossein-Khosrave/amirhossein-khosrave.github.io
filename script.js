...
(function () {
    ...
    function updateThemeUI(theme) {
        const btn = document.getElementById('themeToggle');
        if (!btn) return;
        const label = btn.querySelector('.theme-label'); // ممکن است null باشد (در صفحه اصلی)
    ...
    // ========================================================================
    // 3. BILINGUAL ENGINE (با انیمیشن نرم)
    // ========================================================================
    ...
    function applyTranslations(lang, animate = true) {
        ...
        if (animate) {
            ...
            setTimeout(() => {
                ...
                // ===== اضافه کردن کد مخفی‌سازی "Professional Journey & Experience" در انگلیسی =====
                ...
                // ===== پایان کد اضافه شده =====
// ===== مخفی کردن eyebrow در صفحه گواهینامه‌ها برای زبان انگلیسی =====
const certsEyebrow = document.querySelector('.page-certificates .eyebrow');
if (certsEyebrow) {
    if (lang === 'en') {
        certsEyebrow.style.display = 'none';
    } else {
        certsEyebrow.style.display = '';
    }
}
// ==========================================================
            }, 400);
...
// ==========================================================
    ...
    function initNavigation() {
        // تنظیم نشانگر صفحه فعال
        setActiveNavItem();
        
        // رویداد کلیک برای لینک‌های ناوبری
        ...
        // همچنین دکمه‌های btn را نیز شامل کنیم
    ...
    // 5.2 Reveal on scroll with staggered delay
    ...
    // 5.5 Career timeline & winding line + hover highlight
    function initCareerTimeline() {
        ...
        if (windingPath) {
            ...
            window.addEventListener('scroll', updateWindingLine, { passive: true });
            updateWindingLine();
        }

        // Hover highlight on path
        const cards = document.querySelectorAll('.page-career .timeline-card');
        if (cards.length && windingPath) {
            cards.forEach(card => {
    ...
    // 5.6 Skills page: cards, filters, search, compact view, empty state, tilt
    function initSkillsPage() {
        const cards = document.querySelectorAll('.page-skills .skill-card');
        if (!cards.length) return;

        // ===== مدیریت پیش‌فرض نمای فشرده در موبایل =====
        function setCompactViewBasedOnWidth() {
            ...
            if (isMobile) {
                ...
                // بستن همه کارت‌ها در حالت compact
        ...
        // گوش‌دادن به تغییر اندازه پنجره (با debounce)
        let resizeTimer;
        ...
        // Tilt effect (only if not reduced motion)
        if (!prefersReducedMotion) {
            cards.forEach(card => {
        ...
        // ==============================================
        // Toggle expand — اصلاح شده برای موبایل
        // ==============================================
        cards.forEach(card => {
            // رویداد کلیک روی خود کارت (بدون شرط compact-view)
            card.addEventListener('click', function (e) {
                // اگر کلیک روی دکمه‌ی toggle بوده، اجازه نمی‌دهیم دوباره اجرا شود (با stopPropagation مدیریت می‌شود)
            ...
            // دکمه‌ی toggle: فقط برای جلوگیری از propagation
            const toggleBtn = card.querySelector('.toggle-btn');
            if (toggleBtn) {
                toggleBtn.addEventListener('click', function (e) {
                    e.stopPropagation(); // مانع از رسیدن کلیک به کارت می‌شود
                });
            }
        });
    ...
    // 5.7 Certificates tilt + hologram
    ...
    // 5.8 Category pills for certificates (scroll to section)
    ...
    // 5.9 Header scroll detection (enhanced) — برای هدر جدید نیازی به تغییر ندارد، اما نگه می‌داریم
    function initHeaderScroll() {
        // هدر جدید fixed است و نیازی به تشخیص اسکرول ندارد، اما برای سازگاری با سایر صفحات تابع را خالی می‌گذاریم
        // در صورت نیاز می‌توان این تابع را برای افزودن افکت‌های دیگر به هدر در آینده استفاده کرد.
    }

    // 5.10 Percentage number converter (fa/en)
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // 6.3 Event listeners for toggles
        ...
        // 6.5 Listen for system theme changes
        ...
        // 6.6 Watch for language changes to re-translate percentages and theme UI
        ...
        // 6.7 Observer for dynamic content (empty state in skills may need re-translation)
        // پس از تغییر زبان، دیکشنری به‌روز می‌شود، اما empty state ممکن است دوباره ساخته شود.
        // در تابع filterCards، empty state با متن ترجمه‌شده ساخته می‌شود.
    });
